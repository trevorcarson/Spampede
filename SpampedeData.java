...
class SpampedeData {
	...
	public BoardCell getNextCellInDir() {
		...
		if(this.currentMode == SnakeMode.GOING_EAST) return this.getEastNeighbor();
		else if(this.currentMode == SnakeMode.GOING_WEST) return this.getWestNeighbor();
		else if(this.currentMode == SnakeMode.GOING_NORTH) return this.getNorthNeighbor();
		else if(this.currentMode == SnakeMode.GOING_SOUTH) return this.getSouthNeighbor();
		else return null;
	}
  ...
  public BoardCell getNextCellFromBFS() {
    ...
    while(!cellsToSearch.isEmpty()){
    	...
    	cellsToSearch.add(getNorthNeighbor(dequeued));
    	cellsToSearch.add(getSouthNeighbor(dequeued));
    	cellsToSearch.add(getEastNeighbor(dequeued));
    	cellsToSearch.add(getWestNeighbor(dequeued));
    }
  ...
  public void reverseSnake() {
	  // Step 1: un-label the head
	  this.getSnakeHead().becomeBody();
	  // Step 2: reverse the body parts
	  LinkedList<BoardCell> copyCells = new LinkedList<BoardCell>();
	  for(int i = 0; i < this.snakeCells.size(); i++) {
		  copyCells.addFirst(this.snakeCells.get(i));
	  }
	  this.snakeCells.clear();
	  while(!copyCells.isEmpty()) {
		  this.snakeCells.addFirst(copyCells.removeLast());
	  }
	  // Step 3: relabel the head
	  this.getSnakeTail().becomeHead();
	  // Step 4: calculate the new direction after reversing!
	  if(this.getNorthNeighbor(this.getSnakeNeck()).equals(getSnakeHead())) {
		  this.setDirectionNorth();
	  }
	  if(this.getSouthNeighbor(this.getSnakeNeck()).equals(getSnakeHead())) {
		  this.setDirectionSouth();
	  }
	  if(this.getWestNeighbor(this.getSnakeNeck()).equals(getSnakeHead())) {
		  this.setDirectionWest();
	  }
	  if(this.getEastNeighbor(this.getSnakeNeck()).equals(getSnakeHead())) {
		  this.setDirectionEast();
	  }
  }
